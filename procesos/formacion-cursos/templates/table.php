...
$cursos = $pdo->query("SELECT * FROM v_formacion_curso");
$index = 0;
?>
 <header class="panel-heading bg-info margin-bottom">
   <h5 class="no-margin text-center">LISTA DE FORMACION CURSOS</h5>
</header>
<div class="table-responsive">
  <table class="table-striped table-hover table-advance table-bordered col-xs-12" id="TabFilter">
    ...
    <tbody>
      ...
      ?>
      <tr>
        <td><?= $row["zc_curso"]; ?> - <?= $row["zf_alias"]; ?></td>
        <td class="flex-around">
          <button class="btn btn-embossed btn-primary editar" data-id="<?=$row["zf_cod"];?>">
            <i class="fui-new"></i>
          </button>
          <button class="btn btn-embossed btn-danger eliminar" data-id="<?=$row["zf_cod"];?>">
            <i class="fui-cross"></i>
          </button>
          <button class="btn btn-embossed btn-inverse imprimir" data-id="<?=$row["zf_cod"];?>">
